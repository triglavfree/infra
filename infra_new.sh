...
set -uo pipefail
# =============================================================================
# INFRASTRUCTURE v11.0.1 (PERPLEXICA EDITION FOR UBUNTU 24.04)
# =============================================================================
...
print_header "INFRASTRUCTURE v11.0.1 (PERPLEXICA)"
...
for dir in "$INFRA_DIR" "$VOLUMES_DIR" "$BIN_DIR" "$LOGS_DIR" "$BACKUP_DIR" "$BACKUP_DIR/cache" "$BACKUP_DIR/snapshots" \
           "$VOLUMES_DIR"/{gitea,torrserver,perplexica} "$QUADLET_USER_DIR"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
...
status_cmd() {
    clear
    print_box "INFRA STATUS v11.0.1"
    ...
    # Perplexica status
    local perp_svc=$(get_service_status "perplexica" "user")
    local perp_ctr=$(get_container_status "perplexica" "user")
    print_metric "Perplexica" "$perp_svc $perp_ctr"
    
    if podman ps --format "{{.Names}}" 2>/dev/null | grep -q "^systemd-perplexica$"; then
        local perp_port=$(podman port systemd-perplexica 2>/dev/null | grep "3000/tcp" | cut -d: -f2 || echo "3080")
        print_metric "" "${MUTED_GRAY}→ http://${server_ip}:${perp_port}${RESET}"
    fi
...
case "${1:-status}" in
    ...
    stop)
        ...
        systemctl --user stop gitea torrserver perplexica 2>/dev/null && echo -e "  ${ICON_OK} Gitea/TorrServer/Perplexica" || echo -e "  ${DIM_GRAY}○ Gitea/TorrServer/Perplexica${RESET}"
    ...
    start)
        ...
        systemctl --user start gitea torrserver perplexica 2>/dev/null && echo -e "  ${ICON_OK} Gitea/TorrServer/Perplexica" || echo -e "  ${ICON_FAIL} Gitea/TorrServer/Perplexica"
    ...
    clear)
        ...
        echo ""
        echo -e "  ${NEON_YELLOW}⚠${RESET} ${BOLD}ВНИМАНИЕ!${RESET} Будут удалены:"
        echo -e "    ${DIM_GRAY}• Все контейнеры и их данные${RESET}"
        echo -e "    ${DIM_GRAY}• Swap файл и настройки${RESET}"
        echo -e "    ${DIM_GRAY}• UFW правила (кроме SSH)${RESET}"
        echo -e "    ${DIM_GRAY}• fail2ban конфигурация${RESET}"
        echo -e "    ${DIM_GRAY}• SSH hardening (будет восстановлен)${RESET}"
        echo -e "    ${DIM_GRAY}• Cron задачи infra${RESET}"
        echo -e "    ${DIM_GRAY}• subuid/subgid записи${RESET}"
        echo ""
        read -rp "  Вы уверены? Введите 'yes' для подтверждения: " CONFIRM
        if [ "$CONFIRM" = "yes" ]; then
            echo ""
            echo -e "  ${NEON_YELLOW}▸ [1/8] Остановка сервисов...${RESET}"
            systemctl --user stop gitea torrserver perplexica 2>/dev/null && echo -e "    ${ICON_OK} Rootless остановлены" || true
            sudo systemctl stop gitea-runner netbird 2>/dev/null && echo -e "    ${ICON_OK} Rootful остановлены" || true
            systemctl --user stop podman-auto-update.timer 2>/dev/null || true
            sudo systemctl stop podman-auto-update.timer 2>/dev/null || true
            
            echo -e "  ${NEON_YELLOW}▸ [2/8] Удаление контейнеров...${RESET}"
            podman rm -f gitea torrserver perplexica 2>/dev/null && echo -e "    ${ICON_OK} Rootless удалены" || true
            sudo podman rm -f gitea-runner netbird 2>/dev/null && echo -e "    ${ICON_OK} Rootful удалены" || true
            
            echo -e "  ${NEON_YELLOW}▸ [3/8] Удаление Quadlet файлов...${RESET}"
            rm -f ~/.config/containers/systemd/gitea.container ~/.config/containers/systemd/torrserver.container ~/.config/containers/systemd/perplexica.container 2>/dev/null && echo -e "    ${ICON_OK} User quadlet" || true
            sudo rm -f /etc/containers/systemd/gitea-runner.container /etc/containers/systemd/netbird.container 2>/dev/null && echo -e "    ${ICON_OK} System quadlet" || true
            sudo rm -f /run/systemd/generator/*.service 2>/dev/null || true
            systemctl --user daemon-reload
            sudo systemctl daemon-reload
            
            echo -e "  ${NEON_YELLOW}▸ [4/8] Удаление Swap...${RESET}"
            if [ -f /swapfile ]; then
                sudo swapoff /swapfile 2>/dev/null || true
                sudo rm -f /swapfile && echo -e "    ${ICON_OK} /swapfile удалён"
                sudo sed -i '/\/swapfile/d' /etc/fstab 2>/dev/null && echo -e "    ${ICON_OK} fstab очищен"
            else
                echo -e "    ${DIM_GRAY}○ Swap файл не найден${RESET}"
            fi
            
            echo -e "  ${NEON_YELLOW}▸ [5/8] Восстановление системных конфигов...${RESET}"
            # UFW - оставляем только SSH
            sudo ufw --force reset >/dev/null 2>&1
            sudo ufw default deny incoming >/dev/null 2>&1
            sudo ufw default allow outgoing >/dev/null 2>&1
            sudo ufw allow 22/tcp comment 'SSH' >/dev/null 2>&1
            sudo ufw --force enable >/dev/null 2>&1
            echo -e "    ${ICON_OK} UFW сброшен (только SSH)"
            
            # fail2ban
            sudo rm -f /etc/fail2ban/jail.local 2>/dev/null && echo -e "    ${ICON_OK} fail2ban config удалён"
            sudo systemctl restart fail2ban 2>/dev/null || true
            
            # SSH - восстанавливаем парольную аутентификацию
            sudo sed -i 's/^PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config 2>/dev/null
            sudo sed -i 's/^#*PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config 2>/dev/null
            sudo systemctl restart sshd 2>/dev/null && echo -e "    ${ICON_OK} SSH восстановлен" || true
            
            # sysctl - удаляем добавленные строки
            sudo sed -i '/vm.swappiness=10/d' /etc/sysctl.conf 2>/dev/null
            sudo sed -i '/net.core.default_qdisc=fq/d' /etc/sysctl.conf 2>/dev/null
            sudo sed -i '/net.ipv4.tcp_congestion_control=bbr/d' /etc/sysctl.conf 2>/dev/null
            echo -e "    ${ICON_OK} sysctl.conf очищен"
            
            echo -e "  ${NEON_YELLOW}▸ [6/8] Удаление пользователей и записей...${RESET}"
            # subuid/subgid - удаляем записи пользователя
            sudo sed -i "/^$USER:/d" /etc/subuid 2>/dev/null && echo -e "    ${ICON_OK} subuid очищен" || true
            sudo sed -i "/^$USER:/d" /etc/subgid 2>/dev/null && echo -e "    ${ICON_OK} subgid очищен" || true
            
            # linger
            sudo rm -f /var/lib/systemd/linger/$USER 2>/dev/null && echo -e "    ${ICON_OK} linger отключён" || true
            
            echo -e "  ${NEON_YELLOW}▸ [7/8] Очистка cron...${RESET}"
            crontab -l 2>/dev/null | grep -v "infra" | crontab - 2>/dev/null && echo -e "    ${ICON_OK} Cron задачи infra удалены" || true
            
            echo -e "  ${NEON_YELLOW}▸ [8/8] Удаление директорий...${RESET}"
            read -rp "    Удалить данные ($INFRA_DIR)? [y/N]: " DEL_DATA
            if [[ "$DEL_DATA" =~ ^[Yy]$ ]]; then
                sudo rm -rf "$INFRA_DIR" 2>/dev/null && echo -e "    ${ICON_OK} $INFRA_DIR удалён"
            else
                echo -e "    ${DIM_GRAY}○ Данные сохранены${RESET}"
            fi
            sudo rm -rf /var/lib/gitea-runner /var/lib/netbird 2>/dev/null && echo -e "    ${ICON_OK} Системные директории удалены" || true
            sudo rm -f /usr/local/bin/infra 2>/dev/null && echo -e "    ${ICON_OK} /usr/local/bin/infra удалён" || true
            ...
            echo -e "${NEON_GREEN}${BOLD}╚════════════════════════════════════════════════════════╝${RESET}"
            echo ""
            echo -e "  ${MUTED_GRAY}Сохранены: SSH ключи, установленные пакеты, BBR (в ядре)${RESET}"
            echo -e "  ${MUTED_GRAY}Для полной очистки системы перезагрузите сервер${RESET}"
        fi
        ;;
    backup)
...
EOENV
    ...
    backup-restore)
        ...
        if [[ ! "${STOP_SERV:-Y}" =~ ^[Nn]$ ]]; then
            systemctl --user stop gitea torrserver perplexica 2>/dev/null
    ...
    update)
        ...
        systemctl --user start gitea torrserver perplexica 2>/dev/null
...
if [ ! -f "$INFRA_DIR/.bootstrap_done" ]; then
    ...
    sudo bash -c "
        ...
        ufw allow 3080/tcp comment 'Perplexica' >/dev/null 2>&1
...
# =============== PERPLEXICA ROOTLESS (QUADLET) ===============
print_step "Создание Perplexica"

# Создаём директорию для данных
mkdir -p "$VOLUMES_DIR/perplexica"
chown "$CURRENT_USER:$CURRENT_USER" "$VOLUMES_DIR/perplexica"
chmod 755 "$VOLUMES_DIR/perplexica"

# Создаём Quadlet файл для Perplexica
# Порт 3000 внутри контейнера маппится на 3080 снаружи (чтобы не конфликтовать с Gitea)
cat > "$QUADLET_USER_DIR/perplexica.container" <<EOF
[Unit]
Description=Perplexica AI Search Engine Container
...
Volume=$CURRENT_HOME/infra/volumes/perplexica:/home/perplexica/data:Z
...
chown "$CURRENT_USER:$CURRENT_USER" "$QUADLET_USER_DIR/perplexica.container"
systemctl --user daemon-reload
systemctl --user start perplexica.service
print_success "Perplexica запущена"
print_url "http://${SERVER_IP}:3080/"
print_info "Настройте API ключи через веб-интерфейс при первом запуске"
...
echo -e "${NEON_GREEN}●${RESET} Perplexica: ${NEON_CYAN}http://$SERVER_IP:3080/${RESET} ${NEON_GREEN}(AI Search Engine)${RESET}"
echo -e "              ${MUTED_GRAY}Настройте API ключи при первом запуске${RESET}"
...
echo -e "Логи:       ${NEON_CYAN}infra logs perplexica${RESET}"
